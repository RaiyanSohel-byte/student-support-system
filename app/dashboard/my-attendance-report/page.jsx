...
const COURSE_ATTENDANCE = [
  ...
  {
    title: "Calculus II",
    code: "MATH201 • Dr. Johnson",
    rate: "81.5%",
    status: "Good",
    stats: { total: 24, present: 19, absent: 3, excused: 2 },
    recent: [
      { date: "Feb 6", type: "present" },
      { date: "Feb 4", type: "present" },
      { date: "Feb 1", type: "absent" },
      { date: "Jan 30", type: "present" },
      { date: "Jan 28", type: "present" },
    ],
  },
  {
  ...
  {
    title: "Physics I",
    code: "PHY150 • Dr. Brown",
    rate: "65.2%",
    status: "Critical",
    stats: { total: 22, present: 14, absent: 7, excused: 1 },
    warning:
      "Critical: You are below the attendance threshold for this course.",
    recent: [
      { date: "Feb 6", type: "absent" },
      { date: "Feb 4", type: "present" },
      { date: "Feb 1", type: "absent" },
      { date: "Jan 30", type: "absent" },
      { date: "Jan 28", type: "present" },
    ],
  },
];
...
const AttendanceReportPage = () => {
  ...
  // Filtering Logic
  const filteredCourses = COURSE_ATTENDANCE.filter((course) => {
    const rate = parseFloat(course.rate);
    if (selectedFilter === "Excellent(90%+)") return rate >= 90;
    if (selectedFilter === "Good(80-89%+)") return rate >= 80 && rate < 90;
    if (selectedFilter === "Warning(70-79%)") return rate >= 70 && rate < 80;
    if (selectedFilter === "Critical(<70%)") return rate < 70;
  ...
  return (
    <div className="p-4 md:p-8 space-y-8 text-slate-800">
      ...
      {/* Filter Bar with Custom Dropdown (Matches my-attendance-dropdown.png) */}
      <div className="relative" ref={dropdownRef}>
        <div
          onClick={() => setIsDropdownOpen(!isDropdownOpen)}
          className="bg-white p-3 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between cursor-pointer hover:border-slate-200 transition-colors max-w-xs"
        >
        ...
        {/* Dropdown Menu */}
        {isDropdownOpen && (
          <div className="absolute left-0 mt-2 w-full max-w-xs bg-white border border-slate-100 rounded-[24px] shadow-xl z-50 p-2 space-y-1 animate-in fade-in zoom-in-95 duration-100">
            {FILTERS.map((filter) => (
              <button
                key={filter}
                onClick={() => {
      ...
      <div className="space-y-6">
            ...
            >
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                ...
                <div className="text-right">
                  <p
                    className={`text-2xl font-black ${parseFloat(course.rate) >= 80 ? "text-green-500" : "text-orange-500"}`}
                  >
                    {course.rate}
                  </p>
        ...
        : <div className="text-center py-20 bg-white rounded-[24px] border border-dashed border-slate-200">
            <p className="text-slate-400 font-medium">
              No courses match this attendance filter.
            </p>
          </div>
        }
      </div>
